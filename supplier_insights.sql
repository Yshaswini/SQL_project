...
Select min(TotalAmt) 
	from 
	(Select sum(TotalAmount) AS TotalAmt 	
	from orders 
	GROUP BY CustomerID)
...
Select CustomerID, SUM(TotalAmount) AS TotalAmt
	from orders
	GROUP BY CustomerID
	HAVING TotalAmt = (Select MIN(TotalAmt)
					   from (Select SUM(TotalAmount) AS TotalAmt
							 from orders
                             GROUP BY CustomerID) Total_by_Customer);
...
Select max(TotalAmt) 
	from (Select sum(TotalAmount) AS TotalAmt 
		  from orders 
          GROUP BY CustomerID) 
          Customer_Total_Amt;  --  max - 117483.39          

Select CustomerID, sum(TotalAmount) AS Total_Amt
	from orders 
    GROUP BY CustomerID
    HAVING Total_Amt = (Select max(Total_Amt) 
					   from (Select sum(TotalAmount) AS Total_Amt
							 from orders
                             GROUP BY CustomerID) Customer_Total_Amt) ;
...
-- 9. List the products whose name starts with C
Select * from Product where PRoductName LIKE 'C%'; 
